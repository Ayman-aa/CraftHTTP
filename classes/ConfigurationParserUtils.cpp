...
bool ConfigurationParser::extractErrorPages(std::ifstream& file, int& currentLineNumber) {
	string line;
...
bool ConfigurationParser::isValidPortSegment(const std::string& segment) {
...
bool ConfigurationParser::isValidIPSegment(const std::string& segment) {
	if (segment.empty()) return false;
...
bool ConfigurationParser::LineIsCommentOrEmpty(std::string& line) {
	if (line.empty()) return true;
	size_t firstNoSpaceTab = line.find_first_not_of(" \t");

	if (firstNoSpaceTab ==std::string::npos) return true;
	string trimCheck = line.substr(firstNoSpaceTab);
	if (trimCheck[0] == '#') return true;
	return false;
}

bool ConfigurationParser::verifyLineFormat(std::string& line, int indentLevel) {
	if (line.empty()) return true;

	size_t firstNoTab = line.find_first_not_of('\t');

	if (firstNoTab ==std::string::npos) return true;
...
bool ConfigurationParser::isValidSecondLevel(std::string& line) {
	/* nbdaw with num of : */
	size_t colonCount = 0;
	...
	if (colonCount != 1 && (line.find("return:") ==std::string::npos)) return false;

	/* na5do pos of colona */
	size_t colonPosition = line.find(':');
	/* nshofo l key */
	kv.key = line.substr(0, colonPosition);
	if (kv.key.empty() || kv.key.find(' ') !=std::string::npos || kv.key.find('\t') !=std::string::npos)
		return false;
	...
	if ((kv.value.empty() || firstNoSpace ==std::string::npos) && kv.key == "error_pages") return true;
	if ((kv.value.empty() || firstNoSpace ==std::string::npos) && kv.key == "cgi_path") return true;
	if (kv.value.empty() || firstNoSpace ==std::string::npos) return false;
	if (firstNoSpace !=std::string::npos && lastNoSpace  !=std::string::npos)
		kv.value = kv.value.substr(firstNoSpace, lastNoSpace - firstNoSpace + 1);
	return true;
}

void ConfigurationParser::syntaxError(int currentLineNumber, const std::string& errorMessage) {
	ostringstream oss;
