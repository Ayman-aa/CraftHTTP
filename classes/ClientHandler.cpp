...
void ClientHandler::readyToReceive() {
    try {
        ...
        if (haeaderGood) {
            if (msg.headers.find("Host") != msg.headers.end()) {
                ServerConfig = Configs.getServerConfig(ServerConfig.host, ServerConfig.ports, msg.headers["Host"].substr(0, msg.headers["Host"].find(":")));
                ParsingReq::serverConfig = ServerConfig;
            } else {
