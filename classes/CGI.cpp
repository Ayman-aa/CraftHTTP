...
void ClientHandler::execCGI(){
	...
	else
	{
		...
		std::string cookie = "";
		if (msg.headers.find("Cookie") != msg.headers.end())
			cookie = msg.headers["Cookie"];
		const char *args[] = {cgi.c_str(), fullLocation.c_str(), postedFileName.c_str(), NULL};
		std::vector<char*> env;

		env.push_back(createCString("QUERY_STRING=" + query));
		env.push_back(createCString("HTTP_COOKIE=" + cookie));
		env.push_back(createCString("REQUEST_METHOD=" + msg.method));
		env.push_back(NULL);

		execve(cgi.c_str(), const_cast<char* const*>(args), const_cast<char* const*>(&env[0]));
		deleteCStrings(env);
		std::exit(1);
	}
}
